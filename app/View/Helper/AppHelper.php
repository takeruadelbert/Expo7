...
class AppHelper extends Helper {
    ...
    function __construct(View $View, $settings = array()) {
        $this->hari[0] = __("Minggu");
        $this->hari[1] = __("Senin");
        $this->hari[2] = __("Selasa");
        $this->hari[3] = __("Rabu");
        $this->hari[4] = __("Kamis");
        $this->hari[5] = __("Jumat");
        $this->hari[6] = __("Sabtu");
    ...
    function cvtTanggalSingkatan($date = null, $now = true) {
        if (!empty($date)) {
            $tgl = date("d", strtotime($date));
            $bulan = $this->getNamaBulanSingkatan(date("m", strtotime($date)));
            $tahun = date("Y", strtotime($date));
        } else if ($now) {
            $tgl = date("d");
            $bulan = $this->getNamaBulanSingkatan(date("m"));
            $tahun = date("Y");
        } else {
            return "-";
        }
    ...
    function cvtHariTanggalEng($date = null, $now = true) {
        if (!empty($date)) {
            $tgl = date("d", strtotime($date));
    ...
    function getNamaBulan($i = null) {
        if ($i == 1) {
            $monthName = __("Januari");
        } elseif ($i == 2) {
            $monthName = __("Febuari");
        } elseif ($i == 3) {
            $monthName = __("Maret");
        } elseif ($i == 4) {
            $monthName = __("April");
            ...
            $monthName = __("September");
        } elseif ($i == 10) {
            $monthName = __("October");
        } elseif ($i == 11) {
            $monthName = __("November");
        } else {
            $monthName = __("December");
        }
        return $monthName;
    }
    
    function getNamaBulanSingkatan($i = null) {
        if ($i == 1) {
            $monthName = __("Jan");
        } elseif ($i == 2) {
            $monthName = __("Feb");
...
//        return ceil((strtotime($curr_year)-strtotime($tmt_year))/4);
    ...
    function getTimeago($ptime) {
        ...
        if ($etime < 1) {
            return sprintf(__('%s %s ago'), "1", "detik");
        }

        $a = array(12 * 30 * 24 * 60 * 60 => __("year"),
        ...
        foreach ($a as $secs => $str) {
            ...
            if ($d >= 1) {
                $r = round($d);
                return sprintf(__('%s %s ago'), $r, $str);
            }
        }
    }
