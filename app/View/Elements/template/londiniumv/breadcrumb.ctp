<div class="breadcrumb-line">
    <ul class="breadcrumb" style="background-color:#0ab34a !important">
        <li><a href="<?php echo Router::url("/admin/dashboard") ?>">Dashboard</a></li>
        <?php
        switch ($url) {
            case "admin/employees/profil":
                ?>
                <li><a href="<?php echo Router::url("/admin/employees/profil") ?>"><?= __("Profile") ?></a></li>
                <?php
                break;
            case "admin/accounts/change_password":
                ?>
                <li><a href="<?php echo Router::url("/admin/accounts/change_password") ?>"><?= __("Change Password") ?></a></li>
                <?php
                break;
        }
    ...
    <ul class="breadcrumb-buttons" style="background-color:#0ab34a !important">
        <li class="dropdown" style="padding:5px 10px 5px;margin-top:5px;margin-bottom:5px">
    ...
    $(document).ready(function () {
        var liveClock = function (servertime) {
            ...
            var formattedDate = function (date) {
                var day = <?= json_encode($this->Html->hari) ?>;
                var month = <?= json_encode($this->Echo->bulan) ?>;
                var dayN = String(currDateTime.getDay());
                var dd = day[parseInt(dayN)];
                var mthN = String(currDateTime.getMonth());
                var mth = month[(parseInt(mthN) + 1)];
