<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/title");
?>
                        ...
                        if (empty($data['rows'])) {
                            ...
                            foreach ($data['rows'] as $item) {
                                ...
                                <tr id="row-<?= $i ?>" class="removeRow<?php echo $item[Inflector::classify($this->params['controller'])]['id']; ?>">
                                    ...
                                    <td class="text-center"><?= $item['Title']['name'] ?></td>
                                    <td class="text-center"><?= ic_rupiah($item['Title']['total_downline']) ?></td>
                                    <td class="text-center">
                                        <?php
                                        $path = !empty($item['Title']['reward_path']) ? "/".$item['Title']['reward_path'] : "/img/no_image.jpg";
                                        ?>
                                        <img src="<?= Router::url($path) ?>" width="70px" height="70px">
                                    </td>
                                    <td class="text-center">
